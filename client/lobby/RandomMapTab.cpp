...
RandomMapTab::RandomMapTab() {
    ...
    labelHeadlineBig = std::make_shared<CLabel>(222, 36, FONT_BIG, EAlignment::CENTER, Colors::YELLOW,
                                                CGI->generaltexth->allTexts[738]);
    labelHeadlineSmall = std::make_shared<CLabel>(222, 56, FONT_SMALL, EAlignment::CENTER, Colors::WHITE,
                                                  CGI->generaltexth->allTexts[739]);

    labelMapSize = std::make_shared<CLabel>(104, 97, FONT_SMALL, EAlignment::CENTER, Colors::WHITE,
                                            CGI->generaltexth->allTexts[752]);
    ...
    addButtonsWithRandToGroup(groupMonsterStrength.get(), monsterStrengthBtns, 2, 4, WIDE_BTN_WIDTH, 248, 251,
                              EMonsterStrength::RANDOM, false);
    ...
    buttonShowRandomMaps->addCallback([&]() {
        parent->toggleTab(parent->tabRandTemplate);
    });
...
void RandomMapTab::updateMapInfoByHost() {
    ...
    for (int i = 0; i < playersToGen; ++i) {
        ...
        if (mapGenOptions->getCompOnlyPlayerCount() != CMapGenOptions::RANDOM_SIZE &&
            i >= mapGenOptions->getPlayerCount()) {
...
void
RandomMapTab::addButtonsWithRandToGroup(CToggleGroup *group, const std::vector<std::string> &defs, int nStart, int nEnd,
                                        int btnWidth, int helpStartIndex, int helpRandIndex, int randIndex,
                                        bool animIdfromBtnId) const {
    ...
    group->addToggle(randIndex, std::make_shared<CToggleButton>(Point(256, 0), RANDOM_DEF,
                                                                CGI->generaltexth->zelp[helpRandIndex]));
    group->setSelected(randIndex);
}

void RandomMapTab::addButtonsToGroup(CToggleGroup *group, const std::vector<std::string> &defs, int nStart, int nEnd,
                                     int btnWidth, int helpStartIndex, bool animIdfromBtnId) const {
    // Buttons are relative to button group, TODO better solution?
    SObjectConstruction obj__i(group);
    int cnt = nEnd - nStart + 1;
    for (int i = 0; i < cnt; ++i) {
        auto button = std::make_shared<CToggleButton>(Point(i * btnWidth, 0),
                                                      animIdfromBtnId ? defs[i + nStart] : defs[i],
                                                      CGI->generaltexth->zelp[helpStartIndex + i]);
...
std::vector<int> RandomMapTab::getPossibleMapSizes() {
    return {CMapHeader::MAP_SIZE_SMALL, CMapHeader::MAP_SIZE_MIDDLE, CMapHeader::MAP_SIZE_LARGE,
            CMapHeader::MAP_SIZE_XLARGE};
}

void RandomMapTab::setParent(CLobbyScreen *newParent) {
