...
SDL_Surface * BitmapHandler::loadBitmapFromMem(const char* fname, ui8* buffer, size_t size, bool setKey) {
...
SDL_Surface * BitmapHandler::loadBitmapFromDir(std::string path, std::string fname, bool setKey)
{
	...
	return loadBitmapFromMem(fname.c_str(), readFile.first.get(), readFile.second, setKey);
}
